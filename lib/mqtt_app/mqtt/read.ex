defmodule MqttApp.Protocol.Read do
  ...
  def parse_fixed_header(<<type::4, flags::bitstring-size(4)>>) do
    case {Opcodes.opcode_to_atom(type), flags} do
      {:reserved, _flags} -> raise "reserved opcode"
      {:publish, <<dup::1, qos::2, retain::1>>} -> {:publish, %Flags{dup: dup, qos: qos, retain: retain}}
      {type, <<0::1, 0::1, 1::1, 0::1>>} when type in [:pubrel, :subscribe, :unsubscribe] -> {type, %Flags{}}
      {type, <<0::4>>} -> {type, %Flags{}}
    end
  end
  ...
  def parse_variable(:connect, <<4::16, "MQTT", version, username::1, password::1, will_retain::1, will_qos::2, will_flag::1, clean_start::1, 0::1, keep_alive::size(2)-unit(8), rest::binary>>) do
      {
        %Variable.Connect{
  ...
  def parse_variable(:puback, <<packet_identifier::size(16), puback_reason_code, rest::binary>>) do
    ...
    {%Variable.Puback{packet_identifier: packet_identifier, puback_reason_code: reason_code}, rest} 
  end
  ...
  def parse_variable(:pubrec, <<packet_identifier::size(16), pubrec_reason_code, rest::binary>>) do
    ...
    {%Variable.Pubrec{packet_identifier: packet_identifier, pubrec_reason_code: reason_code}, rest} 
  end
  ...
  def parse_variable(:pubrel, <<packet_identifier::size(16), pubrel_reason_code, rest::binary>>) do
    ...
    {%Variable.Pubrel{packet_identifier: packet_identifier, pubrel_reason_code: reason_code}, rest} 
  end
  ...
  def parse_variable(:pubcomp, <<packet_identifier::size(16), pubcomp_reason_code, rest::binary>>) do
    ...
    {%Variable.Pubcomp{packet_identifier: packet_identifier, pubcomp_reason_code: reason_code}, rest} 
  end
  ...
  @spec parse_variable(opcode:: atom, data:: binary, qos:: integer) :: {values:: any, rest:: binary}
  ...
  @spec parse_properties(opcode:: atom, data:: binary, map, remaining:: integer) :: {map, rest:: binary}
  ...
  @spec parse_property(opcode:: atom, map:: map, data:: binary) :: {map, consumed:: integer, rest:: binary}
  ...
  defp parse_property(opcode, map, <<2, data::size(32), rest::binary>>) when opcode in [:publish, :will] do
  ...
  defp parse_property(opcode, map, <<17, data::size(32), rest::binary>>) when opcode in [:connect, :connack, :disconnect] do
  ...
  defp parse_property(opcode, map, <<21, rest::binary>>) when opcode in [:connect, :connack, :auth] do
    {:ok, data, consumed, rest} = Utils.decode_utf8(rest) 
    {put_if_not_exists!(map, :authentication_method, data), 1 + consumed, rest}
  end

  defp parse_property(opcode, map, <<22, rest::binary>>) when opcode in [:connect, :connack, :auth] do
    {:ok, data, consumed, rest} = Utils.decode_binary(rest) 
  ...
  defp parse_property(opcode, map, <<31, rest::binary>>) when opcode in [:connack, :puback, :pubrec, :pubrel, :pubcomp, :suback, :unsuback, :disconnect, :auth] do
    {:ok, data, consumed, rest} = Utils.decode_utf8(rest) 
    {put_if_not_exists!(map, :reason_string, data), 1 + consumed, rest}
  end

  defp parse_property(opcode, map, <<33, data::size(16), rest::binary>>) when opcode in [:connect, :connack] do
    {put_if_not_exists!(map, :receive_maximum, data), 3, rest}
  end

  defp parse_property(opcode, map, <<34, data::size(16), rest::binary>>) when opcode in [:connect, :connack] do
  ...
  defp parse_property(opcode, map, <<38, rest::binary>>) when opcode in [:connect, :connack, :publish, :will, :properties, :puback, :pubrec, :pubrel, :pubcomp, :subscribe, :suback, :unsubscribe, :unsuback, :disconnect, :auth] do
  ...
  defp parse_property(opcode, map, <<39, data::size(32), rest::binary>>) when opcode in [:connect, :connack] do
  ...
  def parse_payload(:connect, %Variable.Connect{} = variable_header, data) do
    ...
    {payload, rest} = variable_header
      |> Map.from_struct()
      |> then(fn map -> for key <- [:will_flag, :user_name_flag, :password_flag], do: {key, Map.get(map, key)} end)
      |> Enum.reduce({%Payload.Connect{client_identifier: client_identifier}, rest}, fn {k, v}, {payload, rest} ->
        if v == 1 do
          case k do
            :will_flag ->
              ...
              {%{payload | will_properties: will_properties, will_topic: will_topic, will_payload: will_payload}, rest}
            :user_name_flag ->
  ...
  def parse_payload(:subscribe, extra, rest) do
    ...
    options = %Payload.Subscribe.Options{retain_handling: retain_handling, rap: rap, nl: nl, qos: qos}
  ...
  @spec parse_packet_testing(data:: binary) :: {opcode:: atom, fixed_flags:: Flags.t, variable_header:: any, properties:: map, payload:: any}
  def parse_packet_testing(data) do
    <<fixed_header_flags::binary-size(1), rest::binary>> = data
    {:ok, length, _, rest} = Utils.decode_variable_length(rest)
    {opcode, flags} = parse_fixed_header(fixed_header_flags)
    IO.puts("Parsed packet opcode: #{opcode}")
    IO.inspect(flags, label: "Parsed flags:")
    IO.puts "Parsed packet length: #{length}"
    {variable_header, rest} = case opcode do
      :publish -> 
        %Flags{qos: qos} = flags
    ...
    IO.inspect(variable_header, label: "Parsed variable header: ")

    {properties, rest} = parse_properties(opcode, rest)
    IO.inspect(properties, label: "Parsed properties: ")
    payload = parse_payload(opcode, variable_header, rest)
    IO.inspect(payload, label: "Parsed payload: ")
    {opcode, flags, variable_header, properties, payload}
  end

  @spec parse_packet(socket:: :gen_tcp.socket()) :: {opcode:: atom, fixed_flags:: Flags.t, variable_header:: any, properties:: map, payload:: any}
  def parse_packet(socket) do
