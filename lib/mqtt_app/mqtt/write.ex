defmodule MqttApp.Protocol.Write do
  ...
  def write_fixed_header(opcode, flags, length) do
    ...
    flags = case opcode do
      :reserved -> raise "reserved opcode"
      :publish ->
  ...
  def write_variable(
    ...
    <<4::16, "MQTT", protocol_version, user_name_flag::1, password_flag::1, will_retain::1, will_qos::2, will_flag::1, clean_start::1, 0::1, keep_alive::size(2)-unit(8)>>
  end

  def write_variable(%Variable.Connack{session_present: session_present, connack_reason_code: connack_reason_code}) do
    <<0::7, session_present::1, ReasonCodes.atom_to_reason_code(connack_reason_code)>>
  end

  def write_variable(%Variable.Publish{topic_name: topic_name, packet_identifier: packet_identifier}) when is_nil(packet_identifier) do
    Utils.encode_utf8(topic_name)
  end

  def write_variable(%Variable.Publish{topic_name: topic_name, packet_identifier: packet_identifier}) do
    <<Utils.encode_utf8(topic_name)::binary, packet_identifier::size(2)-unit(8)>>
  end

  def write_variable(%Variable.Puback{packet_identifier: packet_identifier, puback_reason_code: puback_reason_code}) do
    <<packet_identifier::size(2)-unit(8), ReasonCodes.atom_to_reason_code(puback_reason_code)>>
  end

  def write_variable(%Variable.Pubrec{packet_identifier: packet_identifier, pubrec_reason_code: pubrec_reason_code}) do
    <<packet_identifier::size(2)-unit(8), ReasonCodes.atom_to_reason_code(pubrec_reason_code)>>
  end

  def write_variable(%Variable.Pubrel{packet_identifier: packet_identifier, pubrel_reason_code: pubrel_reason_code}) do
    <<packet_identifier::size(2)-unit(8), ReasonCodes.atom_to_reason_code(pubrel_reason_code)>>
  end

  def write_variable(%Variable.Pubcomp{packet_identifier: packet_identifier, pubcomp_reason_code: pubcomp_reason_code}) do
  ...
  def write_properties(opcode, props) do
    properties = Enum.reduce(props, <<>>, fn {k, v}, acc -> 
  ...
  defp write_property(opcode, :payload_format_indicator, value) when opcode in [:publish, :will] do
    <<1, value>>
  end
  ...
  defp write_property(opcode, :subscription_identifier, value) when opcode in [:publish, :subscribe] do
    Enum.reduce(value, <<>>, fn identifier, acc -> 
      <<acc::binary, 11, Utils.encode_variable_length(identifier)::binary>>
    end)
  end

  defp write_property(opcode, :session_expiry_interval, value) when opcode in [:connect, :connack, :disconnect] do
  ...
  defp write_property(opcode, :authentication_method, value) when opcode in [:connect, :connack, :auth] do
    <<21, Utils.encode_utf8(value)::binary>>
  end

  defp write_property(opcode, :authentication_data, value) when opcode in [:connect, :connack, :auth] do
  ...
  defp write_property(opcode, :reason_string, value) when opcode in [:connack, :puback, :pubrec, :pubrel, :pubcomp, :suback, :unsuback, :disconnect, :auth] do
  ...
  defp write_property(opcode, :user_property, value) when opcode in [:connect, :connack, :publish, :will, :properties, :puback, :pubrec, :pubrel, :pubcomp, :subscribe, :suback, :unsubscribe, :unsuback, :disconnect, :auth] do
    Enum.reduce(value, <<>>, fn {string1, string2}, acc -> 
      <<acc::binary, 38, Utils.encode_utf8(string1)::binary, Utils.encode_utf8(string2)::binary>>
    end)
  end
  ...
  def write_payload(%Payload.Connect{} = payload) do
    Enum.reduce([:client_identifier, :will_properties, :will_topic, :will_payload, :user_name, :password], <<>>, fn field, acc -> 
  ...
  def write_payload(%Payload.Subscribe{topic_filters: [{topic_filter, %Payload.Subscribe.Options{retain_handling: retain_handling, rap: rap, nl: nl, qos: qos}} | topic_filters]}) do
    topic_filter = Utils.encode_utf8(topic_filter)
    next_payload = write_payload(%Payload.Subscribe{topic_filters: topic_filters})
    <<topic_filter::binary, 0::2, retain_handling::2, rap::1, nl::1, qos::2, next_payload::binary>>
  end
  ...
  @spec write_packet_testing(opcode:: atom, flags:: Flags.t, variable:: any, properties:: map, payload:: any) :: binary
  def write_packet_testing(opcode, flags, variable, properties, payload) do
    data = write_variable(variable)
    IO.inspect(data, label: "Writing variable header:")
    properties = write_properties(opcode, properties)
    IO.inspect(properties, label: "Writing properties:")
    data = <<data::binary, properties::binary>>
    payload = write_payload(payload)
    IO.inspect(payload, label: "Writing payload:")
    data = <<data::binary, payload::binary>>
    length = byte_size(data)
    IO.puts("Writing packet length: #{length}")
    fixed = write_fixed_header(opcode, flags, length)
    IO.inspect(fixed, label: "Writing fixed header:")
    data = <<fixed::binary, data::binary>>
    data
  end

  @spec write_packet(socket:: :gen_tcp.socket(), opcode:: atom, flags:: Flags.t, variable:: any, properties:: map, payload:: any) :: :ok
  def write_packet(socket, opcode, flags, variable, properties, payload) do
    data = write_variable(variable)
    data = <<data::binary, write_properties(opcode, properties)::binary>>
    data = <<data::binary, write_payload(payload)::binary>>
    length = byte_size(data)
    data = <<write_fixed_header(opcode, flags, length)::binary, data::binary>>
